...
public class WFirmantesOld extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<FirmantesFiltroOld> filterBI;
	private BeanItemContainer<FirmantesOld> itemsBIC;
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<FirmantesFiltroOld>(new FirmantesFiltroOld());
		itemsBIC = new BeanItemContainer<FirmantesOld>(FirmantesOld.class,
				new ArrayList<FirmantesOld>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													FirmantesOld item = (FirmantesOld) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				FirmantesOld item = (FirmantesOld) itemsGRD.getSelectedRow();
				item.getNumero();
	...
	private void loadData() {
		try {
			...
			List<FirmantesOld> items = queryData();

			itemsBIC.removeAllItems();

			for (FirmantesOld item : items) {
	...
	private List<FirmantesOld> queryData() {
		try {
			...
			List<FirmantesOld> items = mockData(limit, offset,
		...
		return new ArrayList<FirmantesOld>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(FirmantesOld item) {
		try {
	...
	List<FirmantesOld> itemsMock = new ArrayList<FirmantesOld>();

	private List<FirmantesOld> mockData(int limit, int offset,
			FirmantesFiltroOld filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				FirmantesOld item = new FirmantesOld();
		...
		ArrayList<FirmantesOld> arrayList = new ArrayList<FirmantesOld>();

		for (FirmantesOld item : itemsMock) {
