...
public class WTalonariosOld extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<TalonariosFiltroOld> filterBI;
	private BeanItemContainer<TalonariosOld> itemsBIC;
	...
	public WTalonariosOld() {
		super();
		init(null);
	}
	
	public WTalonariosOld(Integer numero) {
		super();
		init(numero);
	}
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<TalonariosFiltroOld>(new TalonariosFiltroOld());
		itemsBIC = new BeanItemContainer<TalonariosOld>(TalonariosOld.class,
				new ArrayList<TalonariosOld>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													TalonariosOld item = (TalonariosOld) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				TalonariosOld item = (TalonariosOld) itemsGRD.getSelectedRow();
				item.getNumero();
	...
	private void loadData() {
		try {
			...
			List<TalonariosOld> items = queryData();

			itemsBIC.removeAllItems();

			for (TalonariosOld item : items) {
	...
	private List<TalonariosOld> queryData() {
		try {
			...
			List<TalonariosOld> items = mockData(limit, offset,
		...
		return new ArrayList<TalonariosOld>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(TalonariosOld item) {
		try {
	...
	List<TalonariosOld> itemsMock = new ArrayList<TalonariosOld>();

	private List<TalonariosOld> mockData(int limit, int offset,
			TalonariosFiltroOld filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				TalonariosOld item = new TalonariosOld();
		...
		ArrayList<TalonariosOld> arrayList = new ArrayList<TalonariosOld>();

		for (TalonariosOld item : itemsMock) {
